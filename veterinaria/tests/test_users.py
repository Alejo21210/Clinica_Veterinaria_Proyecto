...
class UserStaffTests(TestCase):
    ...
    def test_user_with_cliente_has_facturas_field(self):
        create_cliente(self.user)
        resp = auth_client(self.staff).get(f'/api/users/{self.user.id}/')
        self.assertIn('num_facturas', resp.data)
